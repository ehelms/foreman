class Puppetclass < ActiveRecord::Base
  ...
  attr_accessible :smart_variables, :smart_variable_ids, :smart_variable_names,
    :smart_class_parameters, :smart_class_parameter_ids, :smart_class_parameter_names,
    :lookup_keys_attributes, :name, :hostgroup_ids, :hostgroup_names
  ...
  def self.search_by_host(key, operator, value)
    ...
    direct = Puppetclass.eager_load(:hosts).where(conditions).pluck('puppetclasses.id').uniq
    hostgroup = Hostgroup.eager_load(:hosts).where(conditions).first
